...
class e2_algebraic_bag extends HandsOnSuite {
 ...
 /**
  ...
  case class FilledBag(content:Int) extends Bag {
    ...
    override def map(function:Int => Int):Bag = FilledBag(function(content))

    override def flatMap(function:Int => Bag):Bag = function(content)

    override def filter(function:Int => Boolean):Bag = if (function(content)) FilledBag(content) else EmptyBag
